...
class MAVLinkThread(QThread):
    ...
    def run(self):
        ...
        _serial_errors = 0          # consecutive serial / OS failures
        _MAX_SERIAL_ERRORS = 5      # tolerate brief glitches before giving up

        while self.running:
            try:
                msg = self.drone.recv_match(blocking=False, timeout=1)
                _serial_errors = 0  # reset on success
                ...
                # ================= RC_CHANNELS =================
                    ...
                    flt_ch = 5
                    if self.drone_commander and self.drone_commander.parameters:
                        p_val = self.drone_commander.parameters.get("FLTMODE_CH", {}).get("value")
                        if p_val is not None:
                            flt_ch = round(float(p_val))
                    
                    if self.current_telemetry_components.get('fltmode_ch', 5) != flt_ch:
                        self.current_telemetry_components['fltmode_ch'] = flt_ch
                        changed = True

                    ch_val = msg_dict.get(f'chan{flt_ch}_raw', 0)
                    if ch_val and ch_val > 800:
                        if self.current_telemetry_components.get('fltmode_pwm', 0) != ch_val:
                            self.current_telemetry_components['fltmode_pwm'] = ch_val
                            changed = True
                ...
                # Windows serial-port disconnect / RPC_E_DISCONNECTED (0x80010108)
                _serial_errors += 1
                print(f"[MAVLinkThread] ⚠️ Serial error ({_serial_errors}/{_MAX_SERIAL_ERRORS}): {e}")
                if _serial_errors >= _MAX_SERIAL_ERRORS:
                    print("[MAVLinkThread] 🔴 Too many serial errors — stopping telemetry thread.")
                    self.running = False
                    break
