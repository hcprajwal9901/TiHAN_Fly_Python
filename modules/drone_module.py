...
class DroneModel(QObject):
    ...
    # ✅ NEW: emitted when flight mode PWM changes dynamically
    fltModePwmChanged = pyqtSignal()
    ...
    def __init__(self):
        ...
        self._telemetry = {
            ...
            'vibration_y': 0.0,
            'vibration_z': 0.0,
            # ✅ NEW: dynamic flight mode channel tracking
            'fltmode_ch': 5,
            'fltmode_pwm': 0,
        }
    ...
    # ✅ NEW: expose dynamic RC channel tracking for FlightMode LIVE badge
    @pyqtProperty(int, notify=fltModePwmChanged)
    def fltModeChannel(self):
        return int(self._telemetry.get('fltmode_ch', 5))

    @pyqtProperty(int, notify=fltModePwmChanged)
    def fltModePwm(self):
        return int(self._telemetry.get('fltmode_pwm', 0))
    ...
    def _on_connection_success(self, drone):
        ...
        # ── Disable port scanning IMMEDIATELY to protect the serial handle ──
        # PnpWatcher and PortScanner query Windows Device Manager which can
        # momentarily destabilise the active USB serial handle → in_waiting crash.
        import modules.port_scan_lock as psl
        psl.DISABLE_PORT_SCANNING = True
        print("[DroneModel] 🔒 Port scanning DISABLED (protecting MAVLink serial)")
        ...
        try:
            ...
            for key, value in data.items():
                ...
                if new_telemetry.get(key) != value:
                    ...
                    # ✅ NEW: track dynamic flight mode channel changes
                    if key in ('fltmode_ch', 'fltmode_pwm'):
                        rc5_updated = True
            ...
            # ✅ NEW: dedicated signal so QML fltMode properties update instantly
            if rc5_updated:
                self.fltModePwmChanged.emit()
    ...
    def addStatusText(self, text):
        try:
            ...
            if len(self._status_texts) > 100:
                self._status_texts.pop(0)
            # Use QTimer.singleShot(0) so the flush is always posted to the
            # main Qt event loop — safe to call from ANY thread (worker, daemon, etc.)
            QTimer.singleShot(0, self._flush_status_texts)
        except Exception as e:
    ...
    def cleanup(self):
        ...
        # ── Re-enable port scanning now that drone is gone ──
        import modules.port_scan_lock as psl
        psl.DISABLE_PORT_SCANNING = False
        print("[DroneModel] 🔓 Port scanning RE-ENABLED (drone disconnected)")
